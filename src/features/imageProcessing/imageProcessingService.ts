...
export class ImageProcessingService {
  ...
  processImage(imageData: ImageData, imageDimension: ImageDimension, thresholdLevel = 1, radius = 0.2): ImageData {
    this._imageBlurService.blurARGB(imageData.data, imageDimension, radius);
    this.dilate(imageData.data, imageDimension);
    this.invertColors(imageData.data);
    this.thresholdFilter(imageData.data, thresholdLevel);
    return imageData;
  }
  ...
  dilate(pixels: Uint8ClampedArray, imageDimension: ImageDimension): void {
    let currIdx = 0;
